'''
Forward-solver:
1. Simulate the diffusion process and compute the Boltzmann-Matano equation
2. Save data file in the path '..\data'
'''
from numpy import *
import numpy as np
...
def derivative(C,x):
    # Compute the derivative of the component
    [N_x,N_c] =C.shape
...
def save_data(x,C,DuDx,Int_c,Coef,Rand0,Rand1,Rand2,Rand3,Rand4):
    [N_x,N_c] = C.shape
    filename = '../data/data_polycase.csv' 
    ...
    csvFile=open("../data/data_polycase_rand.csv",'w',newline='')
    try:
        writer=csv.writer(csvFile)
    ...
    filename = '../data/data_polycase_true.csv' 
    csvFile=open(filename,'w',newline='')
    try:
        writer=csv.writer(csvFile)
