...
x,C,DuDx,Int_c = import_Matano(3)
##flux = import_true_flux(3)
##D_poly = import_poly_diff(1,3)
D = import_true_diff(3)
##D = D[:,iComp-1,:]
...
##plt.plot(DuDx[id_tr,2], label = 'dc3',linewidth = 4.0, linestyle = '-')
...
id_stable = list(range(600,1000,20))
...
model.sub_models[0].ghost_train_adam(C[id_tr,:], d11_DiagMamtano[id_tr,:], niteration=6000, lr=0.0005)
...
model.train_adam(C[id_tr,:], DuDx[id_tr,:], intC[id_tr,:], 20000, lr=0.0005)
model.train_adam(C[id_tr,:], DuDx[id_tr,:], intC[id_tr,:], 20000, lr=0.0005)
...
try:
    ...
    writer.writerow(('D1','D2','D3','D1 var','D2 var','D3 var'))
    for i in range(N_x):
        # writer.writerow((ypred1[i].item() * intC_std.item(),
        #                  ypred2[i].item() * intC_std.item(),
        #                  ypred3[i].item() * intC_std.item(),
        #                  yvar1[i].item() * intC_std.item()**2,
        #                  yvar2[i].item() * intC_std.item()**2,
        #                  yvar3[i].item()*intC_std.item()**2))
