...
# v03: use the updated ELBO from Hensman's paper, GP for bigh data.
# %%
import torch
...
os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'     # in case code donot work in MAC OS
...
class ghost_cigpr(nn.Module):
    def __init__(self, X, N_ghost, ymean=0, ystd=1.):
        ...
        self.log_beta = nn.Parameter(torch.ones(1))
    ...
    def forward(self, Xte):
        ...
        # OPTION1: inducing points with noise
        ...
        # OPTION2: inducing points without noise
        Sigma = self.K_cross(self.X, self.X) + JITTER * torch.eye(self.X.size(0))
        kx = self.K_cross(Xte, self.X)
    ...
    def negative_log_likelihood_givenXY(self, X, Y):
        ...
        # OPTION1: consider y_i independently
        ...
        # OPTION2: L1 bound
        Kmm = self.K_cross(self.X, self.X) + JITTER * torch.eye(self.X.size(0))
        Knm = self.K_cross(X, self.X)

        L = torch.cholesky(Kmm)
        Linv_times_Knm = torch.cholesky_solve(Knm, L)

        K_tile = self.K_cross(X, X) - Linv_times_Knm.t() @ Linv_times_Knm

        loss = 0.5 * self.log_beta.exp().pow(-1) * K_tile.trace()

        ypred, yvar = self.forward(X)
        prob = torch.distributions.multivariate_normal.MultivariateNormal(ypred.flatten().float(),
                                                                          self.log_beta.exp().pow(-1) * torch.eye(self.X.size(0)) )
        loss = loss - prob.log_prob(Y.flatten().float())
    ...
    def ghost_train_adam(self, X, Y, niteration=10, lr=0.001):
        ...
        for i in range(niteration):
            ...
            L = torch.cholesky(Kmm)
            Linv_times_Knm = torch.cholesky_solve(Knm.t(), L)
            ...
            ypred, yvar = self.forward(X)
            prob = torch.distributions.multivariate_normal.MultivariateNormal(ypred.flatten().float(),
                                                                              self.log_beta.exp().pow(-1) * torch.eye(
                                                                                  X.size(0)))
            loss = loss - prob.log_prob(Y.flatten().float())
...
if __name__ == "__main__":
    ...
    model.ghost_train_adam(xtr, ytr, 1000, lr=0.001)
