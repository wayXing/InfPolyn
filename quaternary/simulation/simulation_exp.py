...
def C2diffCoef(C,Rand0,Rand1,Rand2):
    # Definition of Diffusion Coefficients
    [N_x,N_c] = C.shape
    diffusion_coefficient = np.ones([N_x,N_c,N_c])
    dummy_c= np.ones([N_c,N_c])
    for i in range(N_x):
        diffusion_coefficient[i,:,:] = Rand0
        for j in range(N_c):
            diffusion_coefficient[i,:,:] = diffusion_coefficient[i,:,:]+np.exp(-C[i,j])*Rand1[:,:,j]+np.cos(C[i,j])*Rand2[:,:,j]
    return diffusion_coefficient

def RK4(C,delta_x,delta_t,Rand0,Rand1,Rand2):
    # Fourth order Runge-Kutta Method
    # 4-stage K1,K2,K3,K4  C = C + (1/6)*delta_t*(K1+2*K2+2*K3+K4)
    [N_x,N_c] = C.shape
    # Diffusion matrix test Type 3:  c dependent
    diffusion_coeffcient = C2diffCoef(C,Rand0,Rand1,Rand2);
    # compute right-hand side derivative for each component
    ddC = c2ddc_FD(diffusion_coeffcient, C, delta_x)
    # First stage of RK4
    K1 = delta_t*ddC;
    # Second stage of RK4
    diffusion_coeffcient = C2diffCoef(C+0.5*K1,Rand0,Rand1,Rand2);
    ddC = c2ddc_FD(diffusion_coeffcient, C+0.5*K1, delta_x)
    K2 = delta_t*ddC;
    # Third stage of RK4
    diffusion_coeffcient = C2diffCoef(C+0.5*K2,Rand0,Rand1,Rand2);
    ddC = c2ddc_FD(diffusion_coeffcient, C+0.5*K2, delta_x)
    K3 = delta_t*ddC;
    # Fourth stage of RK4
    diffusion_coeffcient = C2diffCoef(C+K3,Rand0,Rand1,Rand2);
...
Rand0 =  0.2+0.6*rd.rand(N_c,N_c)
Rand1 =  rd.rand(N_c,N_c,N_c)
Rand2 =  rd.rand(N_c,N_c,N_c)
dummy_m = np.eye(N_c,N_c)*12+np.ones([N_c,N_c])
# Build Symmetric Matrix
Rand0 = sqrt(Rand0*Rand0.T)*1e-5
Rand0 = Rand0 * dummy_m
for i in range(N_c):  
    Rand1[:,:,i] = sqrt(Rand1[:,:,i]*Rand1[:,:,i].T)*1e-7
...
for i in range(2,N_t+1):
    ...
    C = RK4(C,delta_x,delta_t,Rand0,Rand1,Rand2)
    # Update the Coefficient
    Coef = C2diffCoef(C,Rand0,Rand1,Rand2)
...
def save_data(x,C,DuDx,Int_c,Coef,Rand0,Rand1,Rand2):
    [N_x,N_c] = C.shape
    filename = '../data2/data2_exp.csv' 
    csvFile=open(filename,'w',newline='')
    try:
        writer=csv.writer(csvFile)
        writer.writerow(('x','c1','c2','c3','dc1','dc2','dc3','flux1','flux2','flux3'))
        for i in range(N_x):
            writer.writerow((x[i,0],C[i,0],C[i,1],C[i,2],DuDx[i,0],DuDx[i,1],DuDx[i,2],Int_c[i,0],Int_c[i,1],Int_c[i,2]))
    finally:
        csvFile.close()
        
    # Save random coefficient 
    csvFile=open("../data2/data2_exp_rand.csv",'w',newline='')
    try:
        writer=csv.writer(csvFile)
    ...
    filename = '../data2/data2_exp_true.csv' 
    csvFile=open(filename,'w',newline='')
    try:
        writer=csv.writer(csvFile)
        writer.writerow(('D11','D21','D31','D12','D22','D32','D13','D23','D33'))
        for i in range(N_x):
            writer.writerow((Coef[i,0,0],Coef[i,1,0],Coef[i,2,0],Coef[i,0,1],Coef[i,1,1],Coef[i,2,1],\
                         Coef[i,0,2],Coef[i,1,2],Coef[i,2,2]))
    finally:
        csvFile.close()
    return 0

flag = save_data(x,C,DuDx,Int_c,Coef,Rand0,Rand1,Rand2)
...
plt.plot(x[:,0],C[:,2],label = 'C3',linewidth = 4.0, linestyle = '-')
...
plt.plot(abs(Int_c-flux),label = 'Matano_error',linewidth = 4.0, linestyle = '-')
