...
class linear_jointGP(nn.Module):
    ...
    def train_adam(self, C, dC, intC, niteration=10, lr=0.001):
        ...
        for i in range(niteration):
            ...
            for i, obj in enumerate(self.sub_models):
                if i == 0:
                    loss = loss - torch.distributions.Normal(1, 0.1).log_prob(self.a[i])# 1 0.1
                else:
                    loss = loss - torch.distributions.Normal(0.1, 0.01).log_prob(self.a[i]) # 0.1 0.01
##                    loss = loss - torch.distributions.Laplace(0, .0001).log_prob(self.a[i]) # const
    ...
    def train_bfgs(self, C, dC, intC, niteration=10, lr=0.001, tolerance_change =1e-7):
        # adam optimizer
        # uncommont the following to enable
        ...
        for i in range(niteration):
            def closure():
                ...
                for i, obj in enumerate(self.sub_models):
                    if i == 0:
                        loss = loss - torch.distributions.Normal(1, 0.1).log_prob(self.a[i])
                    else:
                        loss = loss - torch.distributions.Normal(0.1, 0.01).log_prob(self.a[i])
    ##                    loss = loss - torch.distributions.Laplace(0, .0001).log_prob(self.a[i]) # const
