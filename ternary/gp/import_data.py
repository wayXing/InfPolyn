...
def import_Matano(N_c):
  ...
  with open('../data1/data1_polycase.csv')as f:
    f_csv = csv.reader(f)
    f_csv = list(f_csv)
...
def import_true_diff(N_c):
  with open('../data1/data1_polycase_true.csv')as f:
    f_csv = csv.reader(f)
    f_csv = list(f_csv)
  # Number of samples
  N_x = len(f_csv)-1
  ...
  for i in range(N_x):
    for j in range(N_c):
      for k in range(N_c):
        D[i,j,k] = float(f_csv[i+1][N_c*k+j])
...
def import_poly_diff(r,N_c):
  ...
  r = str(r)
  with open('../data1/data1_polycase_poly'+r+'.csv')as f:
    f_csv = csv.reader(f)
    f_csv = list(f_csv)
  # Number of samples  
  N_x = len(f_csv)-1
  ...
  for i in range(N_x):
    for j in range(N_c):
      for k in range(N_c):
        D_poly[i,j,k] = float(f_csv[i+1][N_c*j+k])
