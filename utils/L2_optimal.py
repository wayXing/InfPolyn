...
'''
L2_optimizer
Compute the optimal by L2-optimization
'''
import torch
...
def fit(Input1,Input2,Output,N_c,r,epoch):
    ...
    for i in range(epoch):
        ...
        loss_value = torch.sqrt(torch.sum((pred - Output)**2))/model.row+ torch.sqrt(torch.sum(model.weights1**2 ))/model.column\
                     +torch.sqrt(torch.sum(model.weights2**2))/model.column
        # The loss function
        loss_value.backward()
        # Backward Alogrithm
        opt.step()
...
def L2(icomp):
    ...
    csvFile=open('../data/L2_optimal_'+str(icomp-1).csv','w',newline='')
    try:
